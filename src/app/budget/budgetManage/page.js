...
export default function BudgetAllocationPage() {
  ...
  const [isAccommodationReserved, setIsAccommodationReserved] = useState(false); // 숙박 예약 여부
  ...
  // 기타 예산 (자동 계산)
  // 이미 예약한 항목은 제외
  const other = totalBudget - (isAirfareReserved ? 0 : airfare) - (isAccommodationReserved ? 0 : accommodation);
      ...
      <main className="p-4 pb-20">
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-lg font-bold" style={{ color: '#50B4BE' }}>
              {isAirfareReserved ? '예약 완료' : formatWon(airfare)}
            </span>
          </div>
          ...
          <div className="relative mb-2">
            <input
              ...
              onChange={(e) => {
                ...
                const accommodationValue = isAccommodationReserved ? 0 : accommodation;
                if (newValue + accommodationValue <= totalBudget - 5) {
                  setAirfare(newValue);
                } else {
                  // 총 예산을 넘으면 자동으로 조정
                  setAirfare(totalBudget - accommodationValue - 5);
                }
              }}
              disabled={isAirfareReserved}
              className={`w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer ${
                isAirfareReserved ? 'opacity-50 cursor-not-allowed' : ''
              }`}
              style={{
          ...
          <div className="flex items-center gap-2 mt-3">
            <button
              onClick={() => setIsAirfareReserved(!isAirfareReserved)}
            ...
            <span className="text-sm text-gray-700">이미 항공권을 예약했습니다</span>
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-lg font-bold" style={{ color: '#50B4BE' }}>
              {isAccommodationReserved ? '예약 완료' : formatWon(accommodation)}
            </span>
          </div>
          ...
          <div className="relative mb-2">
            <input
              ...
              onChange={(e) => {
                const newValue = parseInt(e.target.value);
                // 총 예산을 넘지 않도록 제한
                const airfareValue = isAirfareReserved ? 0 : airfare;
                if (airfareValue + newValue <= totalBudget - 5) {
                  setAccommodation(newValue);
                } else {
                  // 총 예산을 넘으면 자동으로 조정
                  setAccommodation(totalBudget - airfareValue - 5);
                }
              }}
              disabled={isAccommodationReserved}
              className={`w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer ${
                isAccommodationReserved ? 'opacity-50 cursor-not-allowed' : ''
              }`}
              style={{
          ...
          <div className="flex items-center gap-2 mt-3">
            <button
              onClick={() => setIsAccommodationReserved(!isAccommodationReserved)}
              className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${
            ...
            <span className="text-sm text-gray-700">이미 숙박시설을 예약했습니다</span>
          </div>
        </div>
