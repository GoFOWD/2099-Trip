...
export default function SearchForm({
  ...
  const searchAirports = (keyword) => {
    ...
    AIRPORTS.forEach((countryItem) => {
      ...
      if (countryItem.country.toLowerCase().includes(keyword)) {
        countryItem.cities.forEach((city) => result.push(...city.airports));
      }
      // 도시명
  ...
  const selectAirport = (airport) => {
    if (activeField === "from") setFrom(airport.code);
      ...
      const idx = Number(idxStr);
      setSegments((prev) =>
        prev.map((s, i) => (i === idx ? { ...s, [which]: airport.code } : s))
      );
    }
    setSuggestions([]);
  };
  ...
  // ✅ 다구간 세그먼트 추가/삭제
  const addSegment = () =>
    setSegments((prev) => [...prev, { origin: "", destination: "", date: "" }]);
  ...
  return (
    <section className="bg-white rounded-[var(--radius-lg)] p-3 shadow-sm relative space-y-2">
      {/* ✅ 여정 타입 선택 */}
      ...
      {tripType !== "MULTI" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <div className="grid grid-cols-2 gap-2 md:col-span-2">
            <div>
              <label className="text-xs text-slate-500">출발 날짜</label>
              <input
                type="date"
                value={departureDate}
            ...
            >
              <div className="relative">
                <input
                  ...
                  value={seg.origin}
                  onChange={(e) =>
                    handleInput(`seg-${idx}-origin`, e.target.value)
                  }
                  className="border rounded-lg p-2 text-sm w-full"
                />
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) =>
                    handleInput(`seg-${idx}-destination`, e.target.value)
                  }
                  className="border rounded-lg p-2 text-sm w-full"
                />
