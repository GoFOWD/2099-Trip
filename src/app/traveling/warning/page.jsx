...
function adaptFromPack(pack) {
  ...
  const police = {
    ...
    phone: pack?.contacts?.police?.phone || pack?.emergency?.police?.number || "",
  ...
  const hospital = {
    ...
    phone:
      pack?.contacts?.hospital?.phone || pack?.emergency?.ambulance?.number || "",
        ...
        (prev ?? []).map((c) => ({
          ...
          distance:
            c?.lat != null && c?.lng != null
              ? `${Math.round(km(latitude, longitude, Number(c.lat), Number(c.lng)) * 10) / 10}km`
              : undefined,
        }))
      );
    };
    ...
    <main className="min-h-screen bg-[#F5F7F9] text-slate-900">
      ...
      >
        <div className="mx-auto flex h-full max-w-md items-center justify-between px-4">
          <h1 className="text-[15px] font-semibold tracking-tight">여행 중</h1>
          <div className="flex items-center gap-3">
            <Image src={EMERGENCY_ICON_SRC} alt="긴급" width={30} height={30} priority />
            <button
      ...
      <section className="mx-auto max-w-md px-4 py-5">
        ...
        <div className="mb-3 grid grid-cols-3 rounded-lg border border-slate-200 bg-white p-1 text-sm">
          {[
            ...
            <button
              ...
              style={tab === t.id ? { backgroundColor: TEAM_MINT } : {}}
            >
              {t.label}
            </button>
          ))}
        ...
        {tab === "contact" && (
          <div className="space-y-3">
            <p className="text-xs text-slate-500">응급상황 시 연락할 수 있는 기관들입니다</p>
            {contacts.map((c) => (
              <ContactCard key={c.id} iconSrc={ICONS[c.id]} {...c} />
            ))}
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-[13px]">
              ...
              <ul className="mt-1 list-disc space-y-1 pl-4 text-[12px] text-slate-700">
                <li>나라에 따라 긴급번호가 다릅니다. (예: 112/911/999…)</li>
                <li>현지 언어가 어려우면 호텔/가이드 도움을 요청하세요.</li>
                <li>긴급 시 정확한 위치 공유가 가장 중요합니다.</li>
              </ul>
            </div>
          </div>
        )}
        ...
        {tab === "phrases" && (
          <div className="space-y-3">
            {phrases.length ? (
              phrases.map((ph, i) => <PhraseRow key={i} kr={ph.kr} local={ph.local} />)
            ) : (
              <>
                <PhraseRow kr="도와주세요!" local="Help me!" />
                <PhraseRow kr="경찰을 불러주세요." local="Please call the police." />
              </>
            )}
          </div>
        )}
        ...
        {tab === "share" && (
          <div className="rounded-xl border border-slate-200 bg-white p-3 text-[13px]">
            <div className="text-sm font-medium">현재 위치 공유</div>
            <p className="mt-1 text-[12px] text-slate-600">
              아래 버튼을 눌러 현재 위치를 메시지 앱으로 공유하거나 지도 앱에서 열 수 있어요.
            </p>
            <div className="mt-3 flex gap-2">
              ...
              <button
                ...
                style={{ borderColor: TEAM_MINT, color: TEAM_MINT, backgroundColor: "white" }}
                onClick={openMap}
              >
                지도에서 보기
              </button>
...
function ContactCard({ shortTitle, title, distance, addr, phone, map, iconSrc }) {
  const ICON_LEFT = 12;
    ...
    <div className="relative rounded-xl border border-slate-200 bg-white p-3 text-left ring-1 ring-slate-100">
      ...
      <div style={{ paddingLeft: TEXT_LEFT }}>
        <div className="flex items-center gap-2">
          <div className="text-[14px] font-semibold text-slate-900">{shortTitle}</div>
          {distance ? (
        ...
        <div className="mt-1 text-[13px] font-medium text-slate-700">{title}</div>
  ...
  if (navigator.share) {
    navigator.geolocation?.getCurrentPosition(
      (pos) => {
        ...
        navigator.share({ title: "내 위치", text: "지금 여기야", url }).catch(() => {});
      },
      () => {
        navigator.share({ title: "내 위치", text: "현재 위치를 확인해줘" }).catch(() => {});
      }
    );
  } else {
    openMap();
