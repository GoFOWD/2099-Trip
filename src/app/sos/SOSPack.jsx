...
export default function SOSPack({ onClose }) {
			...
			.then(j => {
				...
				if (DEBUG) {
					...
					console.table({
						total: fixed.length,
						has_AU_in_final: !!AU,
						AU_region: AU?.region || null,
						has_NZ_in_final: !!NZ,
						NZ_region: NZ?.region || null
					});
				}
			})
	...
	useEffect(() => {
		...
		fetch(url, { cache: 'no-store' })
			...
			.catch(() => {
				...
				if (DEBUG)
					console.log(
						'[SOS] cities from fallback only for',
						code,
						'=>',
						fbList
					);
	...
	useEffect(() => {
		...
		loadPack(currentId)
			.then(d => {
				setData(d);
				if (DEBUG) console.log('[SOS] loaded pack', currentId, d);
			})
	...
	const continentCountries = useMemo(() => {
		...
		if (continent === 'Oceania' && arr.length === 0) {
			const fallback = countries.filter(c =>
