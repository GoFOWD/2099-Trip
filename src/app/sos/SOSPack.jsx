...
const _toKey = (raw) => {
  ...
  if (
    ...
    v.includes('australia and oceania') ||
    v.includes('australia & oceania') ||
    v.includes('australia/new zealand') ||
    v.includes('australia and new zealand') ||
    v.includes('australia & new zealand') ||
    v.includes('pacific') ||
    v.includes('태평양') ||
    v.includes('oceania') ||
...
function _forceOceaniaByNameOrCode(c) {
  ...
  if (code === 'AU' || name.includes('australia') || name.includes('호주'))
    return 'Oceania';
  if (code === 'NZ' || name.includes('new zealand') || name.includes('뉴질랜드'))
    return 'Oceania';
  return null;
}
...
const labelMap = {
  ...
  fire: '소방',
  all_services: '통합(112)',
  all_services_mobile: '통합(모바일)',
  poison_control: '독극물',
  mental_health_crisis: '정신건강',
  tourist_police: '관광경찰',
  coast_guard: '해안경비',
  gendarmerie: '헌병',
  gas: '가스',
...
/* ========================
      컴포넌트 본문
======================== */
export default function SOSPack() {
  // 상태
  ...
  function openTranslate(service) {
    ...
    switch (service) {
      case 'google':
        url = `https://translate.google.com/?sl=auto&tl=${to}&text=${q}&op=translate`;
        break;
      case 'papago':
        url = `https://papago.naver.com/?sk=auto&tk=${to}&st=${q}`;
        break;
      case 'deepl':
        url = `https://www.deepl.com/translator#auto/${to}/${q}`;
        break;
      default:
        return;
    }
  ...
  useEffect(() => {
    ...
    fetch(url, { cache: 'no-store' })
      ...
      .then((m) => {
        setFallbackMap(m || {});
        if (DEBUG) console.log('[SOS] fallback cities loaded:', m);
      })
    ...
    fetch(url, { cache: 'no-store' })
      ...
      .then((j) => {
        ...
        const norm = raw.map((c) => {
          const code = String(c?.code ?? c?.country_code ?? '')
            .replace(/\uFEFF/g, '')
            .trim()
            .toUpperCase();
          ...
          const region = String(
            c?.region ?? c?.continent ?? c?.regionName ?? '',
          )
            .replace(/\uFEFF/g, '')
            .trim();
          return { ...c, code, name, ko, region };
        });
        if (!norm.some((x) => x.code === 'AU'))
          norm.push({
            code: 'AU',
            name: 'Australia',
            ko: '호주',
        ...
        const fixed = norm.map((c) => {
          ...
          if (c.code === 'AU' || v.includes('australia') || v.includes('호주'))
            return { ...c, region: 'Oceania' };
          if (
            c.code === 'NZ' ||
            v.includes('new zealand') ||
            v.includes('뉴질랜드')
          )
            return { ...c, region: 'Oceania' };
          return c;
  ...
  const continentCountries = useMemo(() => {
    const arr = countries
      .filter((c) => getContinentKey(c) === continent)
      .sort((a, b) => (a.ko || a.name).localeCompare(b.ko || b.name, 'ko'));
    if (continent === 'Oceania' && arr.length === 0) {
      const fb = countries
        .filter((c) => ['AU', 'NZ'].includes(String(c?.code || '').toUpperCase()))
        .sort((a, b) => (a.ko || a.name).localeCompare(b.ko || b.name, 'ko'));
      return fb;
    }
    return arr;
    ...
    <div className="sospack sos-wrap">
        ...
        <section className="sos-stage">
          <div className="sos-bar">
            <div className="sos-bar-left">
              <button className="sos-btn" onClick={() => setStep('continent')}>
                ← 이전
              </button>
            ...
            {filteredCountries.map((c) => (
              ...
              >
                <div className="tile-top">
                  {c.code || '??'} · {c.region || c.continent || c.regionName}
                </div>
      ...
      {step === 'city' && (
        <section className="sos-stage">
          <div className="sos-bar">
            <div className="sos-bar-left">
              <button className="sos-btn" onClick={() => setStep('country')}>
                ← 이전
              </button>
              <div className="sos-crumb">
                {CONTINENT_LABEL[continent] || continent} / {country}
              </div>
            </div>
            <button className="sos-btn" onClick={handleClose}>
              닫기 ✕
          ...
          {cities.length > 0 ? (
            ...
            <div className="sos-empty">
              ...
              <div style={{ marginTop: 8 }}>
                <button className="sos-btn mint" onClick={() => setStep('view')}>
                  국가 정보 보기
                </button>
              </div>
            </div>
        ...
        <section className="sos-stage">
          <div className="sos-bar">
            <div className="sos-bar-left">
              <button
                className="sos-btn"
                onClick={() => setStep(cities.length ? 'city' : 'country')}
              >
                ← 이전
              </button>
              <div className="sos-crumb">
                {CONTINENT_LABEL[continent] || continent} / {country}
                {cityId ? ` / ${prettyCity(cityId)}` : ''}
              </div>
            </div>
            <button className="sos-btn" onClick={handleClose}>
              닫기 ✕
          ...
          {currentId && (
            <div style={{ margin: '6px 0 10px' }}>
              <Link
                href={`/sos/help?c=${country || ''}&city=${cityId || ''}`}
                className="sos-btn btn-primary"
                prefetch={false}
              >
                🆘 긴급 도움
              </Link>
            ...
            <div className="sos-grid">
                ...
                {data.embassy_kr ? (
                  <div className="emb">
                    ...
                    {data.embassy_kr.phones?.map((p, i) => (
                      <div className="emb-row" key={i}>
                        ☎️ {p.label}: <a href={`tel:${p.number}`}>{p.number}</a>
                      </div>
                    ))}
                    {data.embassy_kr.email && (
                      <div className="emb-row">✉️ {data.embassy_kr.email}</div>
                    )}
                    {data.embassy_kr.fax && (
                      <div className="emb-row">📠 {data.embassy_kr.fax}</div>
                    )}
                  </div>
                ) : (
                ...
                <div className="phrase-list">
                  {(data?.phrases?.[tab] || []).map((row, i) => (
                    <div className="phrase" key={i}>
                      {row.title && (
                        <div className="ko" style={{ marginBottom: 4 }}>
                          {row.title}
                        </div>
                      )}
              ...
              <section className="sos-card">
                ...
                <div style={{ display: 'grid', gap: 8 }}>
                  ...
                  >
                    <label style={{ fontSize: 13, color: '#0B5E5E' }}>
                      목표 언어
                    </label>
                    <select
                      value={tTo}
                    ...
                    <div style={{ flex: 1 }} />
                    <button className="sos-btn" onClick={() => openTranslate('google')}>
                      Google 번역
                    </button>
                    <button className="sos-btn" onClick={() => openTranslate('papago')}>
                      Papago
                    </button>
                    <button className="sos-btn" onClick={() => openTranslate('deepl')}>
                      DeepL
                    </button>
                  </div>
...
:root { --mint:#50B4BE; --mint-hover:#3aa6b0; --bg:#E9FFFA; --ink:#063B3B; }
...
.sos-btn.mint { background:var(--mint); color:#fff; border-color:transparent; font-weight:800; }
.btn-primary{ background:var(--mint); color:#fff !important; border-color:transparent !important; font-weight:800; }
...
.tile { text-align:left; background:#fff; border:1px solid #8BE3D4; border-radius:16px; padding:14px 14px; cursor:pointer; transition: border-color .15s, transform .05s, box-shadow .15s, background .15s, color .15s; color:#063B3B; }
.tile.mint { background:var(--mint); border-color:transparent; color:#ffffff; box-shadow:0 2px 10px rgba(0,0,0,.06); }
.tile.mint:hover { background:var(--mint-hover); transform: translateY(-1px); box-shadow:0 8px 20px rgba(80,180,190,.25); }
.tile.mint.on    { outline:2px solid #2d8f99; outline-offset:2px; }
.tile-top  { opacity:.9; font-size:12px; margin-bottom:2px; }
.tile-main { font-weight:800; letter-spacing:-.2px; }
.tile-sub  { opacity:.9; font-size:12px; margin-top:2px; }
...
/* 칩(도시, 탭 등) : 흰 배경 + 팀 민트 글자 / 선택시 굵게 */
.chip{ background:#fff; color:var(--mint); border:1px solid var(--mint); padding:8px 12px; border-radius:999px; font-weight:700; transition: background .15s, border-color .15s; }
.chip.on,.chip:hover{ background:#E7F6F8; border-color:#49aab4; box-shadow:0 0 0 2px rgba(80,180,190,.15) inset; }
...
.phrase{ position: relative; background: #fff; border: 1px solid #8BE3D4; border-radius: 12px; padding: 12px 12px 12px 40px; line-height:1.6; }
.phrase::before{
