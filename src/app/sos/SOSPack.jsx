...
export default function SOSPack({ onClose }) {
	...
	useEffect(() => {
		...
		fetch(url, { cache: 'no-store' })
			...
			.then(j => {
				...
				const fbList = Array.isArray(fallbackMap[code])
					? fallbackMap[code]
					: [];
	...
	const continentCountries = useMemo(() => {
		const arr = countries
			...
			.sort((a, b) =>
				(a.ko || a.name).localeCompare(b.ko || b.name, 'ko')
			);
		if (continent === 'Oceania' && arr.length === 0) {
			const fb = countries
				.filter(c =>
					['AU', 'NZ'].includes(String(c?.code || '').toUpperCase())
				)
				.sort((a, b) =>
					(a.ko || a.name).localeCompare(b.ko || b.name, 'ko')
				);
		...
		<div className='sos-wrap'>
					...
					<div className='country-grid'>
						{filteredCountries.map(c => (
							<button
								...
								{c.ko && (
									<div className='tile-sub'>{c.name}</div>
								)}
							</button>
						))}
					</div>
			...
			{step === 'view' && (
				<section className='sos-stage'>
					<div className='sos-bar'>
						<div className='sos-bar-left'>
							<button
								className='sos-btn'
								onClick={() =>
									setStep(cities.length ? 'city' : 'country')
								}>
								← 이전
							</button>
							<div className='sos-crumb'>
					...
					{!data && !err && (
						<div className='sos-loading'>불러오는 중…</div>
					)}
					...
					{data && (
						<div className='sos-grid'>
							...
							<section className='sos-card'>
								...
								{data.embassy_kr ? (
									<div className='emb'>
										<div className='emb-name'>
											{data.embassy_kr.name}
										</div>
										<div className='emb-row'>
											📍 {data.embassy_kr.address}
										</div>
